...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- Shrink or expand the master area vertically
    , ((modm .|. shiftMask, xK_u     ), sendMessage MirrorShrink)
    , ((modm .|. shiftMask, xK_i     ), sendMessage MirrorExpand)

    -- Toggle fullscreen/noborder
...
myLayout = avoidStruts $ mkToggle (NBFULL ?? NOBORDERS ?? EOT) $ mySpacing 4 ( 
...
main = do
    ...
    xmproc <- spawnPipe "xmobar -x 1 ~/.config/xmobar/xmobarrc"
